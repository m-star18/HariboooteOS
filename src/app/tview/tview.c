...
void HariMain(void) {
    ...
    char str[30], *p, *q = 0, *r = 0;

    /* コマンドライン解析 */
    api_cmdline(str, 30);
    for (p = str; *p > ' '; p++) { }  /* スペースが来るまで読み飛ばす */
    for (; *p != 0; ) {
        p = skipspace(p);
        if (*p == '-') {
            if (p[1] == 'w') {
...
char *lineview(int win, int w, int y, int xskip, unsigned char *p, int tab, int lang) {
    ...
    if (x > 0) {
        str[x] = 0;
        api_putstrwin(win + 1, 8, y, 0, x, str);
    }
    return p;
}

int puttab(int x, int w, int xskip, char *str, int tab) {
    for (;;) {
        if (0 <= x && x < w)
            str[x] = ' ';
        x++;
